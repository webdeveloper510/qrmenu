...
/*var checkPrivacyPolicy = function(){
    ...
    console.log('ffff');
    if ($('#privacypolicy').is(':checked')==false && $('#special_offers').is(':checked')==false) {
...
$("#checkout_customer_email_field").change(function() {
    $(document.body).find('#checkoutPayField').val($(document.body).find('#checkout_customer_email_field').val());
});
...
$("#custom_tip").change(function() {
    $(document.body).find('#checkoutCustomTipsField').val($(document.body).find('#custom_tip').val());
});
...
/*$(document.body).on('change','#privacypolicy,#special_offers',function(){
    ...
    $(this).val(1);
    if(this.checked){
        var pol =$('#privacypolicy').val();
        var spe =$('#special_offers').val();
        if(pol==1 && spe==1)
        {
            $(document.body).find('.paymentbutton').attr('disabled',false);
        }
        else
        {
            $(document.body).find('.paymentbutton').attr('disabled',true);
        }
