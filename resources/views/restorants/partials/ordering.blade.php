...
@include('partials.input',['id'=>'vat','name'=>__('VAT percentage( for all menu items )'),'placeholder'=>__('Item VAT percentage'),'value'=>$restorant->vat==""?$restorant->getConfig('default_tax_value',0):$restorant->vat,'required'=>false,'type'=>'number','min'=>0])
...
@if(config('app.ordering'))
    ...
    <!-- @include('partials.input',['id'=>'tips','name'=>__('Tips'),'placeholder'=>__('Tips'),'value'=>$restorant->tips==""?$restorant->getConfig('default_tax_value',0):$restorant->tips,'required'=>false,'type'=>'text']) -->
...
<script>
    ...
    function delete_tips(restaurant_id,tips){
        ...
        $.ajax({
            ...
            type : 'POST',
            data : {
                'restaurant_id' : restaurant_id,'tips':tips,'_token':'{{ csrf_token() }}'
            },
            //dataType   :'json',
