...
<div class="card-body">
    <h6 class="heading-small text-muted mb-4">{{ __('Restaurant information') }}</h6>
     ...
     <div class="pl-lg-4">
         ...
         <h4>{{ $order->checkout_customer_email_field }}</h4>
    ...
    @if($order->custom_tip==0)
     <h4>{{ __("Tips") }}:  @money(($order->tips/100) * ($order->delivery_price+$order->order_price_with_discount), $currency,true)  </h4>
     <h4>{{ __("Grand Total") }}: @money( $order->delivery_price+$order->order_price_with_discount+$vat_total+($order->tips/100) * ($order->delivery_price+$order->order_price_with_discount), $currency,true)</h4>
    @else
        <h4>{{ __("Tips") }}:  @money($order->tips, $currency,true)  </h4>
        <h4>{{ __("Grand Total") }}: @money( $order->delivery_price+$order->order_price_with_discount+$vat_total+$order->tips, $currency,true)</h4>
    @endif
